...
arr = np.genfromtxt('household_power_consumption.txt', names=True, dtype=[date, time, '<f8', '<f8', '<f8', '<f8', '<f8', '<f8', '<f8'], delimiter=';', converters={0: lambda x: datetime.strptime(x, '%d/%m/%Y').date(), 1: lambda x: datetime.strptime(x, '%H:%M:%S').time()}, max_rows=500000)
arr = arr[~np.isnan(arr['Voltage'])]

df = pd.read_csv("household_power_consumption.txt", header=0, sep=';', na_values='?', dtype={'Global_active_power':'<f8','Global_reactive_power':'<f8','Voltage':'<f8','Global_intensity':'<f8','Sub_metering_1':'<f8','Sub_metering_2':'<f8','Sub_metering_3':'<f8'}, converters={0: lambda x: datetime.strptime(x, '%d/%m/%Y').date(), 1: lambda x: datetime.strptime(x, '%H:%M:%S').time()}, nrows=500000)
df = df.dropna()
